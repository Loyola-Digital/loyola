...
| 39.I3 | Inácio M2 | **Camada row-level de venda** (por transação: id, criado_em_utc, valor bruto, plataforma, UTMs lead+venda) — dedup por ID, fuso UTC→BRT, coorte D+x, amarração lead↔venda. Decisão de contrato/PII pendente. | Backlog (decisão) |
| 39.I4 | Inácio M-listas | **Listas cumulativas (Front/Comunidade) + cross-launch** (recompra entre lançamentos, match por hash de email/telefone server-side). | Backlog (decisão PII) |
