...
export function LeadScoringAdsetTable({
  projectId,
  funnelId,
  stageId,
  days,
}: LeadScoringAdsetTableProps) {
  const [sortKey, setSortKey] = useState<SortKey | null>(null);
  const [sortDirection, setSortDirection] = useState<SortDirection>(null);
  const [columnWidths, setColumnWidths] = useState<Record<string, number>>({});
  const resizeRef = useRef<{ column: string; startX: number; startWidth: number } | null>(null);

  const { data, loading, error } = useLeadScoringAdsetBreakdown(
    projectId,
    funnelId,
    stageId,
    days,
  ...
  const getSortedRows = () => {
    ...
    const sorted = [...data.rows].sort((a, b) => {
      ...
      switch (sortKey) {
        case "adsetName": {
          const aName = a.adsetName || "";
          const bName = b.adsetName || "";
    ...
    return (
      <div className="rounded-xl border border-red-200 bg-red-50/50 dark:bg-red-950/20 p-5">
        <p className="text-sm text-red-700 dark:text-red-400">
          Erro ao carregar breakdown de adsets: {error}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-xl border border-border/30 bg-card/60 p-5">
        <p className="text-sm text-muted-foreground">
          Nenhum dado de utm_medium encontrado na planilha de pesquisa.
        </p>
      </div>
    );
  }
  ...
  if (rows.length === 0) {
    return (
      <div className="rounded-xl border border-border/30 bg-card/60 p-5">
        <p className="text-sm text-muted-foreground">
          Nenhum adset com leads associados.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-border/30 bg-card/60 p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold">Breakdown de Leads por Adset</h3>
      </div>
        ...
        <div className="rounded-md border overflow-y-auto max-h-[500px]">
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="sticky left-0 bg-background z-10 relative group" style={{ width: getColumnWidth("adsetName", 160) }}>
                    <Button
                      variant="ghost"
                      size="sm"
                    ...
                    <div
                      onMouseDown={handleMouseDown("adsetName", 160)}
              ...
              <TableBody>
                {paginatedRows.map((row) => (
                  <TableRow key={row.utmMedium}>
                    ...
                    {bandIds.map((bid) => (
                      <TableCell key={`${row.utmMedium}-${bid}-pct`} className="text-right text-sm">
              ...
              <TableBody>
                {paginatedRows.map((row) => (
                  <TableRow key={row.utmMedium}>
                    ...
                    {bandIds.map((bid) => (
                      <div key={`row-${row.utmMedium}-${bid}`} className="contents">
                        <TableCell className="text-right text-sm">
