    ...
    <div className="space-y-3">
      ...
      {sheetLoading ? (
        ...
        <div className="rounded-lg border border-dashed border-border/50 p-6 text-center">
          ...
          <p className="text-sm text-muted-foreground">Nenhuma planilha de ascensão conectada.</p>
          <p className="text-xs text-muted-foreground mt-1">
            Conecte a planilha do produto de ascensão pra ver quem comprou o perpétuo e depois comprou a ascensão.
          </p>
        </div>
      ) : dataLoading ? (
        ...
        <>
          <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
            ...
            <Kpi label="Ascensões (após)" value={fmtNumber(data.upsells)} highlight />
            <Kpi label="Taxa de ascensão" value={fmtPercent(data.taxaUpsell)} highlight />
            <Kpi label="Faturamento Ascensão" value={formatCurrency(data.faturamentoHighTicket)} />
            <Kpi label="Ticket médio Ascensão" value={formatCurrency(data.ticketMedioHighTicket)} />
          </div>
          ...
          {data.compradores.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border/50 bg-muted/30 text-xs text-muted-foreground">
                      ...
                      <th className="py-2 px-3 text-left font-medium whitespace-nowrap">Compra Ascensão</th>
                      <th className="py-2 px-3 text-right font-medium">Compras Ascensão</th>
                      <th className="py-2 px-3 text-right font-medium">Valor Ascensão</th>
                    </tr>
                  </thead>
                  <tbody>
            ...
            <p className="text-sm text-muted-foreground">
              Ninguém do perpétuo comprou a ascensão depois ainda.
            </p>
          )}
        </>
      )}
