...
import { ExternalLink, UserPlus, TrendingUp, ImageIcon } from "lucide-react";
    ...
    <div className="rounded-xl border border-border/30 bg-card/60 p-5 space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span
            className="inline-flex items-center gap-1 rounded-full border border-border/40 bg-muted/30 px-2 py-0.5 text-[10px] text-muted-foreground"
            title="A Meta Graph API só expõe `follows` por post para fotos e carrosséis (FEED). Reels são ignorados."
          >
      ...
      {isLoading ? (
        ...
        <div className="py-6 text-center text-sm text-muted-foreground space-y-1">
          {data && data.totalFeedInPeriod === 0 ? (
            <>
              <p>Nenhum post FEED no período.</p>
              <p className="text-[10px]">
                {data.totalReelsSkipped > 0
                  ? `${data.totalReelsSkipped} Reels ignorados (Meta não expõe follows pra Reels via API).`
                  : "Publique fotos ou carrosséis pra ver o ranking."}
              </p>
            </>
          ) : (
            <>
              <p>Nenhum post FEED retornou dados de seguidores ainda.</p>
              <p className="text-[10px]">
                A métrica pode levar 24-48h pra estabilizar após a publicação.
              </p>
            </>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          {(data.totalFeedInPeriod > data.totalWithData || data.totalReelsSkipped > 0) && (
            <p className="text-[10px] text-muted-foreground text-center pt-2">
              {data.totalWithData} de {data.totalFeedInPeriod} posts FEED retornaram a métrica
              {data.totalReelsSkipped > 0
                ? ` · ${data.totalReelsSkipped} Reels ignorados`
                : ""}
            </p>
          )}
        </div>
