...
export function MindCard({ mind }: MindCardProps) {
  return (
    <Link href={`/minds/${mind.id}`} className="group flex h-full">
      <div className="relative flex flex-col w-full rounded-xl border border-border/50 bg-card/80 backdrop-blur-sm p-4 transition-all duration-200 hover:border-brand/30 hover:shadow-[0_0_20px_-4px_hsl(47_98%_54%/0.15)] hover:bg-card">
        <div className="flex items-start gap-3">
          <MindAvatar name={mind.name} className="ring-2 ring-border/30 group-hover:ring-brand/30 transition-all duration-200 shrink-0" />
        ...
        <div className="mt-auto pt-3 flex flex-wrap gap-1.5 min-h-[26px]">
